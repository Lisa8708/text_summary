#! -*- coding: utf-8 -*-
# bert做Seq2Seq任务，采用UNILM方案：基于文章内容自动生成标题。
# 通过R-Drop增强模型的泛化性能
...
def load_data(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for (i,l) in enumerate(f):
            ...
            #             print("doc:", doc)
            try:
                title, content = doc['title'], doc['abst']
...
# data path
data_path = '../data/csl_title/'

# 加载数据集
train_data = load_data(data_path + 'csl_title_train.json')
valid_data = load_data(data_path + 'csl_title_dev.json')
test_data = load_data(data_path + 'csl_title_test.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if metrics['bleu'] > self.best_bleu:
            ...
            model.save_weights('./csl.best_model.weights')  # 保存模型
...
print("train:{}, valid:{}, test:{}".format(len(train_data), len(valid_data), len(test_data))) # data = generator*batch
print("train:{}".format(len(train_generator)))
...
if __name__ == '__main__':
    ...
    model.load_weights('./csl.best_model.weights')
    with open('csl.valid.pred', 'w+') as fw:
        for title, content in tqdm(valid_data):
            pred_title = ''.join(autotitle.generate(content, 1)).lower()
            fw.write('\t'.join([title, content, pred_title]) + '\n')